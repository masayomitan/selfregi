...
type AccountQuery struct {
	...
	withManagedAccountDetails *AccountDetailQuery
	withOwner                 *VisitorQuery
	withFKs                   bool
...
// QueryManagedAccountDetails chains the current query on the "managed_account_details" edge.
func (aq *AccountQuery) QueryManagedAccountDetails() *AccountDetailQuery {
	query := &AccountDetailQuery{config: aq.config}
...
func (aq *AccountQuery) QueryOwner() *VisitorQuery {
	query := &VisitorQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withManagedAccountDetails: aq.withManagedAccountDetails.Clone(),
...
// WithManagedAccountDetails tells the query-builder to eager-load the nodes that are connected to
// the "managed_account_details" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithManagedAccountDetails(opts ...func(*AccountDetailQuery)) *AccountQuery {
	query := &AccountDetailQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withManagedAccountDetails = query
	return aq
}

// WithOwner tells the query-builder to eager-load the nodes that are connected to
// the "owner" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithOwner(opts ...func(*VisitorQuery)) *AccountQuery {
	query := &VisitorQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [2]bool{
			aq.withManagedAccountDetails != nil,
			aq.withOwner != nil,
		}
	)
	if aq.withOwner != nil {
	...
	if query := aq.withManagedAccountDetails; query != nil {
		if err := aq.loadManagedAccountDetails(ctx, query, nodes,
			func(n *Account) { n.Edges.ManagedAccountDetails = []*AccountDetail{} },
			func(n *Account, e *AccountDetail) {
				n.Edges.ManagedAccountDetails = append(n.Edges.ManagedAccountDetails, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := aq.withOwner; query != nil {
		if err := aq.loadOwner(ctx, query, nodes, nil,
			func(n *Account, e *Visitor) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadManagedAccountDetails(ctx context.Context, query *AccountDetailQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccountDetail)) error {
	...
	query.withFKs = true
	query.Where(predicate.AccountDetail(func(s *sql.Selector) {
		s.Where(sql.InValues(account.ManagedAccountDetailsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_managed_account_details
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_managed_account_details" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_managed_account_details" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AccountQuery) loadOwner(ctx context.Context, query *VisitorQuery, nodes []*Account, init func(*Account), assign func(*Account, *Visitor)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Account)
	for i := range nodes {
		if nodes[i].visitor_managed_accounts == nil {
			continue
		}
		fk := *nodes[i].visitor_managed_accounts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "visitor_managed_accounts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
