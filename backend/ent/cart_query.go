...
type CartQuery struct {
	...
	withCartDetails *CartDetailQuery
...
// QueryCartDetails chains the current query on the "cart_details" edge.
func (cq *CartQuery) QueryCartDetails() *CartDetailQuery {
	query := &CartDetailQuery{config: cq.config}
...
func (cq *CartQuery) QueryOwner() *VisitorQuery {
	query := &VisitorQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CartQuery) Clone() *CartQuery {
	...
	return &CartQuery{
		...
		withCartDetails: cq.withCartDetails.Clone(),
...
// WithCartDetails tells the query-builder to eager-load the nodes that are connected to
// the "cart_details" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CartQuery) WithCartDetails(opts ...func(*CartDetailQuery)) *CartQuery {
	query := &CartDetailQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCartDetails = query
	return cq
}

// WithOwner tells the query-builder to eager-load the nodes that are connected to
// the "owner" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CartQuery) WithOwner(opts ...func(*VisitorQuery)) *CartQuery {
	query := &VisitorQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CartQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Cart, error) {
	...
	if query := cq.withCartDetails; query != nil {
		if err := cq.loadCartDetails(ctx, query, nodes,
			func(n *Cart) { n.Edges.CartDetails = []*CartDetail{} },
			func(n *Cart, e *CartDetail) { n.Edges.CartDetails = append(n.Edges.CartDetails, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withOwner; query != nil {
		if err := cq.loadOwner(ctx, query, nodes, nil,
			func(n *Cart, e *Visitor) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CartQuery) loadCartDetails(ctx context.Context, query *CartDetailQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *CartDetail)) error {
	...
	query.withFKs = true
	query.Where(predicate.CartDetail(func(s *sql.Selector) {
		s.Where(sql.InValues(cart.CartDetailsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.cart_cart_details
		if fk == nil {
			return fmt.Errorf(`foreign-key "cart_cart_details" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "cart_cart_details" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CartQuery) loadOwner(ctx context.Context, query *VisitorQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *Visitor)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Cart)
	for i := range nodes {
		if nodes[i].visitor_carts == nil {
			continue
		}
		fk := *nodes[i].visitor_carts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "visitor_carts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
